...
String? describeProfilePictureError(File file) {
  if (file.lengthSync() > maxProfilePictureBytes) return 'Image must be smaller than 3MB.';
  return null;
}
...
Future<String> updateProfilePicture(String uid, File file, List<String> extraDocPaths) async {
  final ref = FirebaseStorage.instance.ref('profile-pictures/$uid/photo');
  await ref.putFile(file, SettableMetadata(contentType: 'image/jpeg'));
  final photoUrl = await ref.getDownloadURL();

  final batch = FirestoreService.batch();
  batch.set(FirestoreService.doc('users/$uid'), {'photoUrl': photoUrl}, SetOptions(merge: true));
  for (final path in extraDocPaths) {
    batch.set(FirestoreService.doc(path), {'photoUrl': photoUrl}, SetOptions(merge: true));
  }
  await batch.commit();
